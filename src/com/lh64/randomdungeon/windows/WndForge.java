...
public class WndForge extends Window {
	...
	private static final String TXT_PROMPT =
		"Select two upgradable items to be reforged into one item with better quality.";
	private static final String TXT_SELECT =
		"Select an item to reforge";
	private static final String TXT_REFORGE =
		"Reforge them";
	
	public WndForge( Mob troll, Hero hero ) {
		...
		btnReforge = new RedButton( TXT_REFORGE ) {
			@Override
			protected void onClick() {
				TrollSmith.upgrade( btnItem1.item, btnItem2.item );
				hide();
			}
		};
		...
		public void onSelect( Item item ) {
			if (item != null) {
				...
				if (btnItem1.item != null && btnItem2.item != null) {
					String result = TrollSmith.verify( btnItem1.item, btnItem2.item );
					if (result != null) {
